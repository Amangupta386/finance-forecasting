...
class _UploadFileButtonState extends State<UploadFileButton> {
  ...
  Future<void> _uploadFile(List<int> bytes) async {
    ...
    request.headers['content-type'] = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet';
    ...
    return Container(
      ...
      child: TextButton(
        ...
        child: Row(
          ...
          children: [
            Icon(MaterialIcons.cloudUpload, size: deviceWidth/75,),
            SizedBox(width: deviceWidth/340),
            Text(Labels.uploadFile, style: textTheme.bodySmall?.copyWith(color: colorTheme.secondary, fontSize: deviceWidth/100)),
          ],
        ),
      ),
    );
