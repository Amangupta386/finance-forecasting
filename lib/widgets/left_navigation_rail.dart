...
class NavigationRailItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final Widget toggleCustomIndicator = IgnorePointer(
      child: Container(
        height: Sizes.s30,
    ...
    Widget customIndicator = MouseRegion(
      cursor: SystemMouseCursors.click,
      child: Container(
        height: Sizes.s30,
        decoration: BoxDecoration(
    ...
    return GestureDetector(
      child: Padding(
        ...
        child: (isSelected) ? toggleCustomIndicator : customIndicator,
      ),
      onTap: () {
        onTap(); // Execute the onTap callback and change the index so that it goes on next indictor.
        GoRouter.of(context).go(url); // Navigate to the specified URL.
      },
    );
  }
}
