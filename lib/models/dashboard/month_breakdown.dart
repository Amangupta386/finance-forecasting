...
class MonthBreakdownModel {
  ...
  void enterMonthBlock( int colIndex) {
    if (colIndex >= 0 && colIndex < monthValues.length)  {
      monthValues[colIndex].monthBlock = true;
    }
  }
  void exitMonthBlock(int colIndex) {
    if (colIndex >= 0 && colIndex < monthValues.length)  {
    monthValues[colIndex].monthBlock = false;
    }
  }

  void enterMonthIcon(int colIndex) {
    if (colIndex >= 0 && colIndex < monthValues.length) {
      monthValues[colIndex].monthIcon = true;
    }
  }
  void exitMonthIcon(int colIndex) {
    if (colIndex >= 0 && colIndex < monthValues.length) {
      monthValues[colIndex].monthIcon = false;
    }
  }
  void onEnterSelectMonthIcon(){
    selectRowMonth = !selectRowMonth;
  }

  void onExitSelectMonthIcon(){
    selectRowMonth = false;
  }
