...
class CustomTextFormField extends StatefulWidget {
  ...
  const CustomTextFormField(
      ...
      this.expenseRowData,
      this.resourceRowData,
      this.monthBreakdownRowData});
    ...
    return Focus(
        ...
        child: Padding(
          ...
          child: TextFormField(
            controller:
                (widget.textFormFieldName == Labels.monthBreakdownTextField)
                    ? widget.monthBreakdownRowData!
                        .monthValues[widget.cellIndex].controller
                    : (widget.textFormFieldName == Labels.expenseMonthTextField)
                        ? widget.expenseRowData!.monthValues[widget.cellIndex]
                            .controller
                        : (widget.textFormFieldName ==
                                Labels.resourceMonthTextField)
                            ? widget.resourceRowData!
                                .monthValues[widget.cellIndex].controller
                            : null,
            validator: TextFieldValidation.validateInput,
            onChanged: (value) {
              if (widget.textFormFieldName == Labels.monthBreakdownTextField) {
                widget.monthBreakdownRowData!.monthValues[widget.cellIndex]
                    .value = value;
              }

              if (widget.textFormFieldName == Labels.expenseMonthTextField) {
                widget.expenseRowData!.monthValues[widget.cellIndex].value =
                    value;
              }

              if (widget.textFormFieldName == Labels.resourceMonthTextField) {
                widget.resourceRowData!.monthValues[widget.cellIndex].value =
                    value;
              }
            },
            maxLength: 11,
            autofocus: true,
            readOnly: (Provider.of<ProjectsProvider>(context).toMoveNavigator)
                ? false
                : true,
            decoration: (widget.textFormFieldName ==
                ...
                ? InputDecoration(
                    ...
                    focusedBorder: OutlineInputBorder(
                        borderSide: BorderSide(
                      color: (widget.monthBreakdownRowData!
                              .monthValues[widget.cellIndex].isValid)
