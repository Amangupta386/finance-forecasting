...
class DashboardProjectDatatable extends StatefulWidget {
  ...
  const DashboardProjectDatatable(
      {super.key, required this.headingText, required this.rowsLength});
...
class _DashboardProjectDatatableState extends State<DashboardProjectDatatable> {
  ...
  Widget build(BuildContext context) {
    ...
    Stack stackMethod(double margin) {
      return Stack(
        children: [
          TotalDataTable(
              rowColLength: widget.rowsLength, headingText: widget.headingText),
          Container(
            margin: EdgeInsets.only(left: margin),
            child: MonthDataTable(
              headingText: widget.headingText,
              horizontalScrollController: arrowButtonScrollController,
              rowColLength: widget.rowsLength,
            ),
          ),
        ],
      );
                      ...
                      child: (widget.headingText ==
                          ...
                          ? Column(
                              children: [
                                ...
                                Flexible(
                                  child: stackMethod(monthBreakdownLeftMargin),
                                ),
                              ],
                            )
                          : Stack(children: [
                              ListView(
                                children: [
                                  ...
                                  stackMethod(leftMarginExpenseResource),
                                  if (widget.headingText !=
                                      Labels.monthByMonthBreakdown)
                                    (projectsProvider.toMoveNavigator == true)
                                        ? Consumer<DashboardProvider>(
                                            builder: (context,
                                                dashboardProvider, child) {
                                              return GestureDetector(
                                                onTap: () {
                                                  dashboardProvider.lastRowBar(
                                                      widget.headingText);
                                                },
                                                child: Container(
                                                  color: (provider.visibleRowCount ==
                                                              provider
                                                                  .dashboardAndProjectResourcesCostData
                                                                  .length &&
                                                          widget.headingText ==
                                                              Labels
                                                                  .resourceCostHeading)
